...
function initGrid(){
	...
	let sceneControls = function() {
		this.rotation = 0.005;
		this.grid = true;
		this.lines = true;
	};
	let sceneCtrl = new sceneControls();
	const gui = new dat.GUI();
	gui.add(sceneCtrl, 'rotation', -0.05, 0.05);
	gui.add(sceneCtrl, 'grid');
	gui.add(sceneCtrl, 'lines');
	...
	camera.position.x = (-maxValue - 1) * distanceBetweenCubes * 0.5;
	camera.position.y = 40;
  camera.position.z = 10;
  scene.position.y = (-maxValue - 1) * 0.5;
	...
	for (const [key, value] of Object.entries(colors)) {
		let x = parseInt(value.substring(1,3), 16);
		let y = parseInt(value.substring(3,5), 16);
		let z = parseInt(value.substring(5,7), 16);
		let material = new THREE.MeshBasicMaterial(
			{color: `rgb(${x}, ${y}, ${z})`}
		);
		let mesh = new THREE.Mesh(geometry, material);
		mesh.position.x = x * distanceBetweenCubes - (maxValue*distanceBetweenCubes/2);
		mesh.position.y = y * distanceBetweenCubes;
		mesh.position.z = z * distanceBetweenCubes - (maxValue*distanceBetweenCubes/2);
		mesh.name = key;
		// console.log(mesh)
		scene.add(mesh);
	}

	const size = maxValue*distanceBetweenCubes;
	const divisions = 25;
	const gridHelper = new THREE.GridHelper( size, divisions );
	
	function createLine(color, origin, destination){
		let lineMaterial = new THREE.LineBasicMaterial( { color: `rgb(${color})` } );
		let points = [];
	...
	function hoverElement(event){
		...
		for (var i = 0; i < intersects.length; i++) {			
			let isCube = intersects[i].object.type == "Mesh";
			if(isCube){
				selected?.scale.set(1, 1, 1);
				intersects[i].object.scale.set(10, 10, 10);
				selected = intersects[i].object;
			}
		}
	}
	
	window.addEventListener('mousemove', hoverElement);
*/
	const red = createLine('255,0,0',[-255/2, 0, -255/2], [255/2, 0, -255/2]);
	const blue = createLine('0,0,255',[-255/2, 0, -255/2], [-255/2, 0, 255/2]);
	const green = createLine('0,255,0',[-255/2, 0, -255/2], [-255/2, 255, -255/2]);
	const cyan = createLine('0,255,255',[-255/2, 0, -255/2], [-255/2, 255, 255/2]);
	const magenta = createLine('255,0,255',[-255/2, 0, -255/2], [255/2, 0, 255/2]);
	const yellow = createLine('255,255,0',[-255/2, 0, -255/2], [255/2, 255, -255/2]);
	const white = createLine('255,255,255',[-255/2, 0, -255/2], [255/2, 255, 255/2]);
	
	const linesGroup = new THREE.Group();
	
	const boxGroup = new THREE.Group();
	boxGroup.add(
		createLine('80,80,80',[255/2, 0, -255/2], [255/2, 255, -255/2]),
		createLine('80,80,80',[255/2, 0, -255/2], [255/2, 0, 255/2]),
		createLine('80,80,80',[255/2, 0, 255/2], [255/2, 255, 255/2]),
		createLine('80,80,80',[-255/2, 0, 255/2], [255/2, 0, 255/2]),
		createLine('80,80,80',[-255/2, 255, -255/2], [255/2, 255, -255/2]),
		createLine('80,80,80',[-255/2, 255, -255/2], [-255/2, 255, 255/2]),
		createLine('80,80,80',[-255/2, 0, 255/2], [-255/2, 255, 255/2]),
		createLine('80,80,80',[255/2, 255, -255/2], [255/2, 255, 255/2]),
		createLine('80,80,80',[255/2, 255, 255/2], [-255/2, 255, 255/2]),
	...
	linesGroup.add(red, blue, green, cyan, magenta, yellow, white, boxGroup);

	function render() {
		if (controls) controls.update();
		requestAnimationFrame(render);
		scene.rotation.y += sceneCtrl.rotation;
		renderer.render(scene, camera);
		sceneCtrl.lines ?	scene.add(linesGroup) : scene.remove(linesGroup);
		sceneCtrl.grid ?	scene.add(gridHelper) : scene.remove(gridHelper);
	}
	render();
